...
/**
 * In reality this class doesn't actually localize the Note in 2D Space but
 * instead
 * calcualtes the disance from the nearest point one a measured line that
 * represents
 * the heading of the robot. Make sure to locale the center of your camera to
 * the
 * bottom left hand corner
 */
public class NoteLocalization {

    // Point A is the position of the head of the robot relative to the camera in
    // photon vision
    ...
    /**
     * takes the angle displacements from photon vision and converts them to an
     * arbitrary
     * coordinate system of 1500 x 1000. Note that this is specific to a field of
     * view of 70 degrees and
     * may require tuning if using any other FOV. Also in this case the x axis is 60
     * degrees in full and
     * the y axis is 40 degrees in full.
     * 
     * @param yawAngle   the displacement as an angle in the x direction that photon
     *                   vision gives you of some point
     * @param pitchAngle the displacement as an angle in the y direction that photon
     *                   vision gives you of some point
     */
    ...
    /**
     * 
     * @param pointC the coordinate of the note. Should already be converted to the
    ...
    public static double getSignedDistanceFromNearestPathToNote(double[] pointC) {
        ...
        // if the point is to the left of the line make the distance from the line
        // negative
    ...
    /**
     * Calcultes the point D along the line AB that creates a perpendicular line CD.
     * Assumes that
    ...
    public static double[] calculatePointD(double[] pointC) {
        ...
        double numerator = (pointB[0] - pointA[0]) * (pointC[0] - pointA[0])
                + (pointB[1] - pointA[1]) * (pointC[1] - pointA[1]);
    ...
    /**
     * performs the same calculation as getSignedDistanceFromNearestPathToNote but
     * is unsigned
     * 
     * @param pointC the coordinate of the note. Should already be converted to the
     *               relative coordinate system
     * @return the unsigned distance from the note to a perpindicular point along
     *         line AB
     */
    ...
    /**
     * provides a custom implementation for finding the best note to target in an
     * array of visible notes.
     * prioritizes the vertical distance from the heading of the robot but still
     * takes horizontal factor into
     * account.
     * 
     * @param notes an array containing all the relative positions of the notes from
     *              photonvision. Should not
    ...
    public static double[] calculateBestTarget(double[][] notes) {
        ...
        for (int i = 0; i < notes.length; i++) {
            ...
            double verticalScalar = 1.5; // prefers the horizontal axis by a factor of 2.25 (verticalScalar^2)
            double scaledDistance = Math
                    .pow(Math.pow(verticalDistance * verticalScalar, 2) + Math.pow(horizontalDistance, 2), 0.5);

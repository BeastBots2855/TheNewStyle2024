...
public class ConfigureButtonBindings {
    public ConfigureButtonBindings(
        ...
        // new Trigger(()->m_driverController.getBButton()).onTrue(new SetLights(m_Led,
        // Colors.green));
        // new Trigger(()->m_driverController.getYButton()).onTrue(new SetLights(m_Led,
        // Colors.yellow));
        // new Trigger(()->m_driverController.getXButton()).onTrue(new SetLights(m_Led,
        // Colors.red));

        // Climb Bindings
        // Activate Climb
        new Trigger(() -> m_driverController.getRightTriggerAxis() != 0)
                .onTrue(new ShooterWristClosedLoop(m_ShooterWrist, PIDSetPoint.kShooterClimb)
                        .alongWith(new IntakeWristClosedLoop(m_IntakeWrist, PIDSetPoint.kIntakeClimb)))
                .whileTrue(new ClimberClimb(m_Climb, () -> m_driverController.getRightTriggerAxis(),
                        m_robotDrive::getPitch));
        // Reverse Climb
        new Trigger(() -> m_driverController.getLeftTriggerAxis() != 0).whileTrue(
                new ClimberClimb(m_Climb, () -> -m_driverController.getLeftTriggerAxis(), m_robotDrive::getPitch));
        ...
        // Open Loop Commands
                ...
                // Wrists
                        ...
                        // ShooterWrist
                        new Trigger(() -> m_operatorController.getRightY() > 0.15 || m_operatorController.getRightY() < -0.15)
                                .whileTrue(
                                        new ShooterWristOpenLoop(m_ShooterWrist, () -> MathUtil
                                                .applyDeadband(m_operatorController.getRightY(), OIConstants.kDriveDeadband)));
                // Shooters
                        // ShooterFire
        ...
        // Closed Loop Commands
                ...
                new Trigger(() -> m_operatorController.getXButton())
                        .onTrue(new SetIntakeGroundShooterIn(m_ShooterWrist, m_IntakeWrist));
                // SetIntakeInShooterIn
                new Trigger(() -> m_operatorController.getYButton())
                        .onTrue(new SetIntakeInShooterIn(m_ShooterWrist, m_IntakeWrist));
                // SetIntakeInShooterAmp
                new Trigger(() -> m_operatorController.getBButton())
                        .onTrue(new SetIntakeInShooterAmp(m_ShooterWrist, m_IntakeWrist));
                // SetIntakeInShooterSpeaker
                new Trigger(() -> m_operatorController.getAButton())
                        .whileTrue(new SetIntakeInShooterSpeaker(m_ShooterWrist, m_IntakeWrist));
                // SetClimbPosition
                new Trigger(() -> m_operatorController.getStartButton())
                        .whileTrue(new SetClimbPosition(m_ShooterWrist, m_IntakeWrist));

        // Non-Driver Controlled Actions
                new Trigger(() -> m_Intake.isTouchingLimitSwitch() && !DriverStation.isAutonomousEnabled())
                        .onTrue(new GroundNoteToIndexer(m_IntakeWrist, m_ShooterWrist, m_Intake, m_Indexer));

                // Set lights to yellow if trying to track a note and can see a note
                new Trigger(() -> PhotonVision.canTrustNoteData() && m_driverController.getLeftBumper())
                        .whileTrue(new SetLights(m_Led, Colors.yellow));
                // Set lights to red if trying to track a note and can't see a note
                new Trigger(() -> !PhotonVision.canTrustNoteData() && m_driverController.getLeftBumper())
                        .whileTrue(new SetLights(m_Led, Colors.red));

                // Set lights to green when contacting note
                new Trigger(() -> m_Intake.isTouchingLimitSwitch()).whileTrue(new SetLights(m_Led, Colors.green))
                        .whileFalse(new RAINBOWS(m_Led));
                // The Forsaken One
        ...
        //Vision
                ...
                new Trigger(() -> m_driverController.getRightBumper()).whileTrue(
                        new AutoSpeakerAlignWithDrive(
                                ...
                                () -> -MathUtil.applyDeadband(m_driverController.getLeftY(), OIConstants.kDriveDeadband),
                                () -> -MathUtil.applyDeadband(m_driverController.getLeftX(), OIConstants.kDriveDeadband))
                        .alongWith(new RunCommand(()-> {while(m_ShooterWrist.isWithinPidTolerance()) {m_Led.setColor(Colors.purple);}})));
        ...
        NamedCommands.registerCommand("SetIntakeGroundShooterIn",
                new SetIntakeGroundShooterIn(m_ShooterWrist, m_IntakeWrist));
        ...
        NamedCommands.registerCommand("SetIntakeInShooterAmp",
                new SetIntakeInShooterAmp(m_ShooterWrist, m_IntakeWrist));
        NamedCommands.registerCommand("SetIntakeInShooterSpeaker",
                new SetIntakeInShooterSpeaker(m_ShooterWrist, m_IntakeWrist));
        NamedCommands.registerCommand("SetVisionModeAuto",
